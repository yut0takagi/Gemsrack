...
function App() {
  ...
  const [tab, setTab] = useState<'gems' | 'kpi'>('gems')
  ...
  return (
    <div className="relative min-h-screen text-slate-100">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
              ...
              <div className="inline-flex rounded-xl border border-white/10 bg-black/25 p-1">
                <button
                  className={[
                    ...
                    tab === 'gems' ? 'bg-white/10 text-slate-100' : 'text-slate-300 hover:bg-white/5',
                  ].join(' ')}
                  onClick={() => setTab('gems')}
                >
                  Gems
                ...
                <button
                  className={[
                    'h-9 rounded-lg px-3 text-sm font-semibold transition',
                    tab === 'kpi' ? 'bg-white/10 text-slate-100' : 'text-slate-300 hover:bg-white/5',
                  ].join(' ')}
                  onClick={() => setTab('kpi')}
                >
                  KPI
                ...
                <a
                  href="/admin/"
                  className="h-9 rounded-lg px-3 text-sm font-semibold text-slate-300 hover:bg-white/5 inline-flex items-center"
                >
                  Admin
                </a>
              </div>
          ...
          <div className="border-t border-white/10 p-5">
            <div className="grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-end">
              ...
              {tab === 'gems' ? (
                ...
                <div className="space-y-1">
                  <div className="text-xs font-medium text-slate-300">表示</div>
                  <div className="flex h-10 items-center rounded-xl border border-white/10 bg-black/20 px-3 text-sm text-slate-300">
                    KPI ダッシュボード
                  </div>
                </div>
              )}
        ...
        {tab === 'kpi' ? (
          <div className="mt-6">
            <MetricsPanel teamId={teamId || undefined} />
          </div>
        ) : (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              {g.summary ? (
                                <div className="mt-1 line-clamp-2 text-xs text-slate-300">{g.summary}</div>
                              ) : (
